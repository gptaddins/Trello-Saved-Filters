...
window.addEventListener('load', () => {
  t.render(async () => {
    ...
    const members = await t.members('id', 'username', 'fullName');
...
async function computeFilteredCardIds(t, filter) {
  ...
  const [listName, username] = await Promise.all([
    filter.status ? (await t.list(filter.status)).name : null,
    filter.assignee ? (await t.member(filter.assignee)).username : null
  ]);
